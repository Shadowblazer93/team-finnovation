-- Create the assets table for storing user-specific assets
...
CREATE TABLE IF NOT EXISTS public.assets (
  ...
  name TEXT NOT NULL,
  category TEXT NOT NULL CHECK (category IN ('stocks', 'bonds', 'real_estate', 'crypto', 'cash', 'private_equity', 'commodities', 'alternatives')),
...
-- Table for tracking individual investment decisions (for consistency scoring)
...
CREATE TABLE IF NOT EXISTS public.daily_sentiment (
  ...
  market_sentiment INTEGER CHECK (market_sentiment >= 1 AND market_sentiment <= 5), -- 1=very bearish, 5=very bullish
